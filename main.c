...
static void generate(const struct parser_dag *dag, FILE *file)
{
	fprintf(file, "typedef double (*sigmoid_t)(double);\n");
	fprintf(file, "double evaluate(sigmoid_t sig) {\n");
	reflect(dag, file);
	fprintf(file, "return sig(t%d);\n}\n", dag->id);
}
...
int main(int argc, char *argv[])
{
	...
	struct jitc *jitc;
	evaluate_t fnc;
	sigmoid_t sigmoidFunction = sigmoid;
	FILE *file;
